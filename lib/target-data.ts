...
export type Collection = { id: string; customerId: string; amount: number; date: string; note?: string; saleId?: string; productId?: string };
...
export const EMPTY_DATA: AppData = { targets: [], customers: [], products: [], sales: [], collections: [], cashOpeningBalance: 0, cashMovements: [], inventoryCounts: [] };
...
export function normalizeAppData(value: unknown): AppData {
  ...
  return {
    ...
    collections: Array.isArray(raw.collections) ? raw.collections.map((collection) => ({ ...collection, amount: toNumber(collection.amount), saleId: typeof collection.saleId === "string" ? collection.saleId : undefined, productId: typeof collection.productId === "string" ? collection.productId : undefined })) : [],
    cashOpeningBalance: toNumber(raw.cashOpeningBalance),
