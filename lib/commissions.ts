...
export function getCommissionSummary(data: AppData, target?: QuarterTarget): CommissionSummary {
  if (!target) return { bothTargetsEligible: false, salesCommission: 0, collectionCommission: 0, nobioteKQuantity: 0, nobioteKValue: 0, nobioteKIsCollected: false, nobioteKRate: 0, nobioteKCommission: 0 };
  ...
  const nobioteKIsCollected = nobioteKValue > 0 && metrics.collectionActual >= nobioteKValue;
...
export function getCashSummary(data: AppData) {
  ...
  return { collections, deposits, expenses, expectedCash: collections - deposits - expenses };
}
