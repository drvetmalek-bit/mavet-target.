...
import { currency, quantityFormat } from "./target-data";
...
async function reportHtml(report: TargetReport) {
  ...
  const rows = report.products.map((product) => `<tr><td>${htmlEscape(product.name)}</td><td>${money(product.salesValue)}</td><td>${quantityFormat(product.quantitySold)}</td><td>${product.unitsRemaining === null ? "—" : quantityFormat(product.unitsRemaining)}</td></tr>`).join("");
